...
if [ $? -eq 0 ]; then
    ...
    # フォールバック: PyPDF2
    bash .claude/skills/common/scripts/convert_pdftotext_fallback.sh "./tmp/shiryou1.pdf" "./tmp/shiryou1.txt"
fi
```
...
# 5. 重要ページのみ抽出
IMPORTANT_PAGES="3,4,5,6,7,10,12,15"
bash .claude/skills/common/scripts/extract_important_pages.sh "./tmp/shiryou1_full.md" "./tmp/shiryou1.md" "$IMPORTANT_PAGES"
```
...
```bash
# 非同期変換
TASK_ID=$(bash .claude/skills/common/scripts/docling_convert_async.sh "./tmp/shiryou1.pdf" 2>&1)
